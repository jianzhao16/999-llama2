...
def askonce(input_text):
    ...
    # Tokenize the input
    inputs = tokenizer(input_text, return_tensors="pt", padding=True).to("cuda:2")
...
def generate_text(text, max_new_tokens=128):
    inputs = tokenizer(text, return_tensors="pt").to("cuda:2")
    outputs = finetuned_model.generate(input_ids=inputs.input_ids, attention_mask=inputs.attention_mask, max_new_tokens=max_new_tokens)
