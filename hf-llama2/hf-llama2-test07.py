...
bb_config_4bit = BitsAndBytesConfig(
    load_in_4bit=True,
...
bb_config_8bit = BitsAndBytesConfig(
    load_in_8bit=True,
    bnb_8bit_use_double_quant=True,  # Adjust these settings as needed
    bnb_8bit_quant_type="fp8",       # Use 'nf4' for better accuracy, or 'fp8' for better speed
    #bnb_8bit_compute_dtype=torch.bfloat16  # This can be set to torch.float16 or torch.bfloat16
    bnb_8bit_compute_dtype=torch.bfloat16  # This can be set to torch.float16 or torch.bfloat16
)
...
device_map = {'': torch.cuda.current_device()}  # Map the model to the current device

llama2_model = LlamaForCausalLM.from_pretrained(
    model_base,
    quantization_config=bb_config_4bit,
    device_map=device_map,  # Ensure the model is loaded on the correct device
    use_cache=False
)
#.to("cuda:0")
...
inputs = tokenizer(text_pad1, return_tensors="pt", padding=True)
...
llama2_model.gradient_checkpointing_enable()

llama2_int8_model=llama2_model
#llama2_int8_model = prepare_model_for_int8_training(llama2_model)
...
print_trainable_parameters(llama2_int8_model)
...
llama2_int8_model.save_pretrained(output_dir_v2)
...
print('begin test fine tune model')
...
print('test fine tune model is done')
...
print('Test fine-tuned model is done')
